...
- **SIP Echo Test Extension (`777`)**: Dialing the virtual extension `777` initiates a zero-media-processing loopback test. The server immediately returns a `200 OK` carrying the caller's identical SDP, forcing standard softphones to stream their RTP packets back to their own receive ports for instant local audio verification.
- **Ringing SDP Stripper & Codec Enforcement**: Added `SipMessage::clearBody()` to purge the SDP body and reset the `Content-Length` to `0` on all outgoing `180 Ringing` responses, resolving early-media loopback echo and ringing hangs on strict devices. Added `SipMessage::enforceG711()` to restrict media codecs strictly to PCMU/PCMA (`0 8 101`), solving dead-air issues when strict intercom policies are enforced.
- **Yealink Phone Remote Control Utility**: Added a robust automated controller utility `scratch/yealink_controller.py` using Action URI commands. It supports legacy CGI endpoint commands and T29G anti-CSRF token secured RSA/AES provisioning uploads to easily automate dials, key simulations, and automated test sequences on the LAN.
